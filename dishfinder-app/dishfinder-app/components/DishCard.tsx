...
export default function DishCard({ item }: { item: any }) {
  ...
  const getTagStyle = (tag: string) => {
    switch(tag) {
      ...
      case 'Rare Dish': return { backgroundColor: 'rgba(168, 85, 247, 0.2)', color: '#c084fc' }; // Vibrant purple!
  ...
  return (
    <TouchableOpacity activeOpacity={0.8} onPress={openMaps} style={styles.cardContainer}>
...
const styles = StyleSheet.create({
  ...
  ratingBadge: { backgroundColor: '#2A2A2A', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  ratingText: { color: '#FFD700', fontWeight: 'bold', fontSize: 12 },
  distanceText: { color: '#888', fontSize: 12, fontWeight: '600' },
  dishName: { fontSize: 22, fontWeight: '800', color: '#FFFFFF', marginTop: 8, letterSpacing: 0.5 },
  restaurantName: { fontSize: 14, color: '#B0B0B0', marginTop: 4 },
  price: { fontSize: 18, fontWeight: '700', color: '#FF5A5F' },
